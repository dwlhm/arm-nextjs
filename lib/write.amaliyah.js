...
module.exports = {
    ...
    writeItemOnItem: async (docs) => {

        try {

            console.log(docs)
            
            const item = await firestore().collection('amaliyah').doc(docs.kategori)
                            .collection('post').doc(docs.post)
                            .collection('item').doc(docs.item).get()

            if (!item.exists) throw { code: 38.1 }

            let order = !item.data().itemsOrder ? [] : item.data().itemsOrder

            const itemAsPost = await item.ref.collection('item').orderBy('itemsNumber', 'desc').limit(1).get()

            //if (!itemAsPost.empty) throw { code: 38.1 }

            let num = itemAsPost.empty ? 0 : itemAsPost.docs.map(it => Number(it.data().itemsNumber) + 1)[0]

            if (Array.isArray(docs.data.body)) {

                for (var i = 0; i < docs.data.body.length; i++) {

                    await item.ref.collection('item').doc('item' + num).create({
                        ...docs.data,
                        body: docs.data.body[i],
                        itemsNumber: num,
                        createdAt: firebase.firestore.FieldValue.serverTimestamp(),
                        author: docs.author
                    })
